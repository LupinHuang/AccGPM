...
// SIU helper kernels
// -----------------------------------------------------------------------------
inline data_t fetch_next_valid_block(
    const data_t *cache,    // 输入：本地数据缓存（存储了图的邻接表数据）
    int size,               // 输入：缓存中数据的总长度
    int upper_bound,        // 输入：索引上限。只保留小于此值的邻居节点（用于算法剪枝）
    int &lane_ptr           // 输入/输出：当前通道的读取指针（引用传递，函数内会修改它）
) {
#pragma HLS INLINE
    update_lane_ptr_loop:
    while (lane_ptr < size) {
        data_t val = cache[lane_ptr];
        lane_ptr += BATCH_SIZE;
        if (val.isPadding()) {
                lane_ptr = size;
                break;
        ...
        if (upper_bound >= 0) {
            ...
            if (base + BCSR_bitmap > upper_bound) {
                ...
                if (keep < 0)
                    keep = 0;
                bcsr_bitmap masked = val.get_bitmap();

                bit_set_zero_loop:
                for (int bit = keep; bit < BCSR_bitmap; ++bit) {
#pragma HLS UNROLL
                    masked[bit] = 0;
                }
                val.set_bitmap(masked);
            }
        }
        if (val.get_bitmap() != 0) {
            return val;
...
#ifndef __SYNTHESIS__
inline void decode_cache_neighbors(
    const data_t *cache,
    int size,
    int upper_bound,
    ...
    for (int idx = 0; idx < size; ++idx) {
        const data_t &blk = cache[idx];
        if (blk.isPadding())
            continue;

        int base = blk.get_index() * BCSR_bitmap;
        if (upper_bound >= 0 && base >= upper_bound)
            break;

        bcsr_bitmap bits = blk.get_bitmap();
...
inline void decode_batch_elements(
    const data_t *batch,
    int batch_size,
    std::vector<int> &neighbors
) {
...
struct IterationDebugRecord {
    int iteration = 0;
    int read_ptr_A_before[BATCH_SIZE];
    int read_ptr_B_before[BATCH_SIZE];
    int read_ptr_A_after[BATCH_SIZE];
    int read_ptr_B_after[BATCH_SIZE];
    int batch_count = 0;
    ap_uint<BATCH_SIZE> maskA = 0;
    ap_uint<BATCH_SIZE> maskB = 0;
    process_data_t last_element_in;
    process_data_t last_element_out;
    data_t inA[BATCH_SIZE];
    data_t inB[BATCH_SIZE];
    data_t out_intersection[BATCH_SIZE];
};

inline std::string mask_to_string(const ap_uint<BATCH_SIZE> &mask) {
    std::string bits;
    bits.reserve(BATCH_SIZE);
    for (int i = BATCH_SIZE - 1; i >= 0; --i) {
        bits.push_back(mask[i] ? '1' : '0');
    }
    return bits;
}

inline void dump_block_summary(const data_t &blk, std::ostream &os) {
    if (blk.isPadding()) {
        os << " PAD";
        return;
    }
    os << ' ' << blk.get_index() << '|';
    bcsr_bitmap bits = blk.get_bitmap();
    for (int bit = BCSR_bitmap - 1; bit >= 0; --bit) {
        os << (bits[bit] ? '1' : '0');
    }
}

inline void dump_lane_array(const char *label, const data_t arr[BATCH_SIZE]) {
    std::cout << "            " << label;
    for (int lane = 0; lane < BATCH_SIZE; ++lane) {
        dump_block_summary(arr[lane], std::cout);
    }
    std::cout << std::endl;
}

inline std::string describe_process_data(const process_data_t &val) {
    if (val.bcsr_element.isPadding())
        return "PAD";
    return std::to_string(val.bcsr_element.get_index());
}

inline std::string format_ptr_array(const int arr[BATCH_SIZE]) {
    std::string repr = "[";
    for (int i = 0; i < BATCH_SIZE; ++i) {
...
inline void dump_intersection_vertices(const data_t arr[BATCH_SIZE]) {
    std::vector<int> tmp;
    decode_batch_elements(arr, BATCH_SIZE, tmp);
    std::cout << "             out verts:";
    if (tmp.empty()) {
        std::cout << " none";
    } else {
...
inline void print_iteration_history(const std::vector<IterationDebugRecord> &history) {
    if (history.empty()) {
        std::cout << "        Iteration trace: <empty>" << std::endl;
        return;
    }
    std::cout << "        Iteration trace (" << history.size() << " steps):" << std::endl;
    for (const auto &rec : history) {
        std::cout << "          [iter " << rec.iteration
                  << "] batch=" << rec.batch_count
                  << " maskA=" << mask_to_string(rec.maskA)
                  << " maskB=" << mask_to_string(rec.maskB)
                  << " readA " << format_ptr_array(rec.read_ptr_A_before)
                  << "->" << format_ptr_array(rec.read_ptr_A_after)
                  << " readB " << format_ptr_array(rec.read_ptr_B_before)
                  << "->" << format_ptr_array(rec.read_ptr_B_after)
                  << " last_in=" << describe_process_data(rec.last_element_in)
                  << " last_out=" << describe_process_data(rec.last_element_out)
                  << std::endl;
        dump_lane_array("laneA", rec.inA);
        dump_lane_array("laneB", rec.inB);
        dump_lane_array("merge", rec.out_intersection);
        dump_intersection_vertices(rec.out_intersection);
    }
}
#endif
