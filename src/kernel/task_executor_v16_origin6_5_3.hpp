...
inline void load_vertex_if_needed(
...
#pragma HLS INLINE off
    if (length == 0) {
        cache.vertex_id = vertex_id;
        cache.length = 0;
        return;
    }
    bool cache_hit = (cache.vertex_id == vertex_id) && (cache.length == length);
    if (!cache_hit) {
        const ap_uint<AXI_WIDE_BITS>* wide_ptr =
            reinterpret_cast<const ap_uint<AXI_WIDE_BITS>*>(gmem_bcsr_data);
        const int wide_idx = start / BLOCKS_PER_WIDE;
        const int lane_offset = start % BLOCKS_PER_WIDE;
        const int num_chunks = (length + lane_offset + BLOCKS_PER_WIDE - 1) / BLOCKS_PER_WIDE;

        data_t unpacked[BLOCKS_PER_WIDE];
#pragma HLS ARRAY_PARTITION variable=unpacked complete

    LOAD_VERTEX:
        for (int chunk = 0; chunk < num_chunks; ++chunk) {
#pragma HLS PIPELINE II=1
            ap_uint<AXI_WIDE_BITS> wide_word = wide_ptr[wide_idx + chunk];
            unpack_wide_word(wide_word, unpacked);

            for (int lane = 0; lane < BLOCKS_PER_WIDE; ++lane) {
#pragma HLS UNROLL
                const int global_pos = chunk * BLOCKS_PER_WIDE + lane;
                const int write_idx = global_pos - lane_offset;
                if (write_idx >= 0 && write_idx < length) {
                    cache.blocks[write_idx] = unpacked[lane];
                }
            }
        }
...
inline void stream_vertex_wide_if_needed(
    const data_t* gmem_bcsr_data,
    int start,
    int length,
    int vertex_id,
    VertexCacheWide& cache,
    hls::stream<ap_uint<AXI_WIDE_BITS>>& wide_out
) {
#pragma HLS INLINE off
    ...
    const int lane_offset = start % BLOCKS_PER_WIDE;
    const int num_chunks = (length + lane_offset + BLOCKS_PER_WIDE - 1) / BLOCKS_PER_WIDE;
    bool hit = (cache.vertex_id == vertex_id) && (cache.start == start) && (cache.length == length) && (cache.num_chunks == num_chunks);
...
STREAM_WIDE:
    for (int chunk = 0; chunk < num_chunks; ++chunk) {
#pragma HLS PIPELINE II=1
        ap_uint<AXI_WIDE_BITS> w = hit ? cache.words[chunk] : wide_ptr[wide_idx + chunk];
        wide_out.write(w);
        if (!hit) {
            cache.words[chunk] = w;
        }
...
inline void pe_load_stage(
    const data_t* gmem_bcsr_data_v1,
    const data_t* gmem_bcsr_data_v2,
    hls::stream<RectangleTask>& task_in,
    hls::stream<ComputeMeta>& meta_out,
    hls::stream<data_t>& v1_data_out,
    hls::stream<data_t>& v2_data_out
) {
#pragma HLS INLINE off

    VertexCache cache_v1;
    VertexCache cache_v2;
    cache_v1.vertex_id = -1;
    cache_v2.vertex_id = -1;
    
    // 显式绑定 Cache 到 BRAM
#pragma HLS BIND_STORAGE variable=cache_v1.blocks type=ram_t2p impl=bram
#pragma HLS BIND_STORAGE variable=cache_v2.blocks type=ram_t2p impl=bram
#pragma HLS ARRAY_PARTITION variable=cache_v1.blocks cyclic factor=16 dim=1
#pragma HLS ARRAY_PARTITION variable=cache_v2.blocks cyclic factor=16 dim=1
...
#pragma HLS PIPELINE off
        ...
        meta.v1_len = size_v1;
        load_vertex_if_needed(gmem_bcsr_data_v1, task.v1_start, size_v1, task.v1, cache_v1);

        // 2. 处理 V2 
        // 但得益于外层 DATAFLOW，这里的加载可以和上一个任务的计算并行
        int size_v2 = task.v2_len;
        if (size_v2 > MAX_BCSR_BLOCKS) size_v2 = MAX_BCSR_BLOCKS;
        meta.v2_len = size_v2;
        load_vertex_if_needed(gmem_bcsr_data_v2, task.v2_start, size_v2, task.v2, cache_v2);

        // 3. 发送元数据
        meta_out.write(meta);

        // 4. 将 Cache 数据 burst 写入流中，供 Computer 使用
        // 合并成单个流水循环，同时向两路流写入，减少顺序写带来的气泡
        int push_max = (size_v1 > size_v2) ? size_v1 : size_v2;
        PUSH_BOTH:
        for (int i = 0; i < push_max; ++i) {
#pragma HLS PIPELINE II=1
            if (i < size_v1) {
                v1_data_out.write(cache_v1.blocks[i]);
            }
            if (i < size_v2) {
                v2_data_out.write(cache_v2.blocks[i]);
            }
        }
    }
}

inline void pe_compute_stage(
    hls::stream<ComputeMeta>& meta_in,
    hls::stream<data_t>& v1_data_in,
    hls::stream<data_t>& v2_data_in,
...
#pragma HLS PIPELINE off
        ...
        // 1. 从流中恢复数据到本地数组
        // Computer 在读这些数据时，Loader 已经在为下一个任务加载数据了
        // 合并成单个流水循环，同时从两路流读入，缩短串行读时间
        int pull_max = (meta.v1_len > meta.v2_len) ? meta.v1_len : meta.v2_len;
        PULL_BOTH:
        for (int i = 0; i < pull_max; ++i) {
#pragma HLS PIPELINE II=1
            if (i < meta.v1_len) {
                local_v1[i] = v1_data_in.read();
            }
            if (i < meta.v2_len) {
                local_v2[i] = v2_data_in.read();
            }
        }

        // 2. 执行核心计算
        int ret = 0;
        siu_intersection_cached_fifo_pipelined(local_v1, meta.v1_len, 
                                               local_v2, meta.v2_len, 
                                               meta.upper_bound, ret);
        
        local_total += ret;
    }

    result_stream.write(local_total);
}
